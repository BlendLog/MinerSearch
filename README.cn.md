...
- 扩展了服务和自启动项的启发式检测（包括对标准系统工具隐藏的项目），减少误报
- 新增注册表键值隔离与恢复。关键系统参数仅进行修复
- 新增在下次重启时删除恶意文件，并在报告中体现
- 未签名文件的 SHA1 无论判定结果如何都会记录
...
点击"打开隔离区"按钮将打开隔离区管理器，您可以在其中完全删除或恢复已隔离的文件、计划任务、服务和注册表项。
