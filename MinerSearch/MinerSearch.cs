    ...
    public class MinerSearch
    {
        ...
        void UnlockFile(string filePath)
        {
            ...
            try //DisableInheritance
            {
                ...
                AuthorizationRuleCollection inheritedRules = fileSecurity.GetAccessRules(true, true, typeof(SecurityIdentifier));
                foreach (FileSystemAccessRule inheritedRule in inheritedRules)
                {
                    fileSecurity.RemoveAccessRule(inheritedRule);
                }
            ...
            {
                Logger.WriteLog($"\t[x] Error disabling inheritance and removing inherited permissions for file: {ex.Message}", Logger.error);
            }
